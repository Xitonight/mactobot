...
dp.onNewMessage(filters.command(["messageid", "id"]), async (ctx) => {
  if (ctx.replyToMessage) {
    await ctx.replyText(md`__**Message id:**__ ${ctx.replyToMessage.id}`);
  } else {
    await ctx.replyText(
      md`
❌ You have to **reply** to a message to get its id!
      `
    );
  }
});

dp.onNewMessage(filters.command(["msgid", "mid"]), async (ctx) => {
  if (ctx.replyToMessage) {
    await ctx.replyText(md`__**Message id:**__ ${ctx.replyToMessage.id}`);
  } else {
    await ctx.replyText(
...
dp.onNewMessage(filters.command(["userid", "uid"]), async (ctx) => {
  if (ctx.replyToMessage) {
    await ctx.replyText(
      md`__**User id:**__ ${await ctx
        ?.getReplyTo()
        .then((msg) => msg?.sender.id)}`
    );
  } else {
...
dp.onNewMessage(filters.command(["fileid", "fid"]), async (ctx) => {
  ...
  if (!reply) {
    await ctx.replyText(
      md`
❌ You have to **reply** to a message with a media to get its file_id!
      `
    );
    return;
  }
  ...
  if (reply.text) {
    await ctx.replyText(
      md`
❌ **Text** messages don't have a file_id!
      `
    );
    return;
  }
  ...
  if (
    reply.media?.type === "audio" ||
    reply.media?.type === "document" ||
    reply.media?.type === "photo" ||
    reply.media?.type === "video" ||
    reply.media?.type === "voice" ||
...
const mod: Module = {
  ...
  commands: ["messageid", "userid", "chatid", "fileid"],
  dispatchers: [dp],
  type: "extra",
};
